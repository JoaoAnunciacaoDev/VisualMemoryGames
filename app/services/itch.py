...
class ItchService:
    async def get_profile(self, access_token: str) -> dict:
        ...
        response = await request_async(
            "GET",
            f"{ITCH_API_URL}/profile",
            headers={"Authorization": f"Bearer {access_token}"},
        )

        if response.status_code != 200:
            response = await request_async("GET", f"https://itch.io/api/1/{access_token}/me")
            if response.status_code != 200:
                raise HTTPException(
    ...
    async def get_owned_keys(self, access_token: str) -> list:
        ...
        if response.status_code != 200:
            response = await request_async(
                "GET", f"https://itch.io/api/1/{access_token}/my-owned-keys"
            )
            if response.status_code != 200:
                raise HTTPException(
    ...
    async def get_my_games(self, access_token: str) -> list:
        ...
        response = await request_async(
            "GET",
            f"{ITCH_API_URL}/profile/games",
            headers={"Authorization": f"Bearer {access_token}"},
        )

        if response.status_code != 200:
            response = await request_async("GET", f"https://itch.io/api/1/{access_token}/my-games")
            if response.status_code != 200:
                return []
