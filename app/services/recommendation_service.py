...
def fetch_recommendations_fallback(genres: List[str], exclude_ids: Set[int], db: Session, count: int = 10) -> List[Game]:
...
class RecommendationService:
    ...
    def get_local_games_by_genres(self, genres: List[str], count: int = 10) -> List[Game]:
        ...
        # Para evitar recomendar "lixo" de cache antigo, filtramos apenas jogos que alguém tenha na biblioteca!
        owned_by_others_subq = self.db.query(UserGame.game_id).distinct()
        all_not_owned = self.db.query(Game).filter(
            Game.id.in_(owned_by_others_subq),
            ~Game.external_id.in_(self.owned_external_ids)
    ...
    def _hybrid_recommendation(self, reference_game: Game = None, genres: List[str] = None, count: int = 10) -> List[Dict]:
        ...
        if len(local_recs) < count:
            ...
            external_recs = fetch_recommendations_fallback(target_genres, self.owned_external_ids, self.db, count=needed)
